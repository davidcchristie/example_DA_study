...
def build_particle_distribution(config_particles):
    ...
    Theta = np.delete(Theta, np.where((R >= config_particles["r_max"]) | (R <= config_particles["r_min"])))
    R = np.delete(R, np.where((R >= config_particles["r_max"]) | (R <= config_particles["r_min"])))


    # make a list of tuples
    particle_list = [(particle_id, ii[0], ii[1]) for particle_id, ii in enumerate(zip(R, Theta))]
