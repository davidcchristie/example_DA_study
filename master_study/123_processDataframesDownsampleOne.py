...
for scaleFact in scaling:
    if ampScale:
        amplitudeDownsampling = scaleFact
    if angleScale:
        angleDownsampling = scaleFact
    ...
    numParticles.append(len(df_downsampled_sim))
...
# track change in DA separately for each angular bin
    ...
    minDAAll.append([minDA])

    meanDA = np.nanmean(DAbyNandTheta, 1)
    meanDAAll.append([meanDA])
    
    maxDA = np.nanmax(DAbyNandTheta, 1)
    maxDAAll.append([maxDA])
    
    lostPartTurnsSetAll.append([lostPartTurns])
    minDAEnd.append( minDA[-1])
    maxDAEnd.append(maxDA[-1])
    meanDAEnd.append(meanDA[-1])

plt.plot(scaling, meanDAEnd)
plt.show()

plt.plot(numParticles, meanDAEnd, 'o')
plt.show()
